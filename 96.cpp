// Day 96 - Q1: Counting Inversions in Array
...
long long mergeSort(vector<int>&a, int l, int r){
    long long inv=0;
    if(l<r){ int m=l+(r-l)/2; inv+=mergeSort(a,l,m); inv+=mergeSort(a,m+1,r); inv+=mergeCount(a,l,m,r); }
    return inv;
}
int main(){
    int n; cin>>n; vector<int> a(n);
    for(int i=0;i<n;i++) cin>>a[i];
    cout<<mergeSort(a,0,n-1)<<endl; return 0;
}
